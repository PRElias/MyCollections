...
Esta pasta contém uma página estática gerada a partir do arquivo `GametimeSummary.csv`, localizado na raiz do projeto.
...
8. O gráfico deve continuar mostrando a divisão por plataforma na própria barra empilhada e na legenda.
9. A tabela deve permitir ordenação por todas as colunas, busca textual e filtro por plataforma.
10. Se os arquivos forem usados pelo PWA, atualizar também o `CACHE_NAME` em `docs/service-worker.js`.
